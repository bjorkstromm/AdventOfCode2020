...
b"
        ...
        let groups = scan input

        groups |> should equal [|3;3;3;1;1|]
        groups |> Array.sum |> should equal 11

    [<Fact>]
    member __.``Puzzle input - Part I`` () =
        let input = "wzaopvscxknyjtiul
belforsgdhuvmwc
...
mfijcthadgnobuew"
        ...
        let count = groups |> Array.length
        let sum = groups |> Array.sum

        output.WriteLine("Count: {0}", count)
        output.WriteLine("Sum: {0}", sum)
...
b"
        ...
        let groups = scan2 input

        groups |> should equal [|3;0;1;1;1|]
        groups |> Array.sum |> should equal 6
