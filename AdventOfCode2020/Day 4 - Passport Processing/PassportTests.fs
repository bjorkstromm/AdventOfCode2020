...
type TrajectoryTests(output: ITestOutputHelper) =
    ...
    [<Fact>]
    member __.``Example Passport Data`` () =
...
iyr:2011 ecl:brn hgt:59in"
        ...
        let validPassports = processData input

        validPassports
        |> Array.map (fun r -> sprintf "%A" r)
        |> Array.iter (fun str -> output.WriteLine(str))

        validPassports |> Array.length |> should equal 2
...
hcl:#602927 iyr:2013 byr:2003 eyr:2023 cid:100"
        ...
        output.WriteLine("Valid passwords {0}", validPassports.Length)

    [<Fact>]
    member __.``Example Passport Data - Part II`` () =
...
iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719"
        ...
        let validPassports = processDataV2 input

        validPassports
        |> Array.map (fun r -> sprintf "%A" r)
        |> Array.iter (fun str -> output.WriteLine(str))

        validPassports |> Array.length |> should equal 4
